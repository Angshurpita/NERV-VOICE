...
router.patch('/me', attachUser, requireAuth, async (req: AuthedRequest, res) => {
...
router.post('/me/password', attachUser, requireAuth, async (req: AuthedRequest, res) => {
  const parsed = z
  ...
  if (problems.length > 0) {
    res.status(400).json({ error: `New password needs ${problems.join(', ')}.` });
    return;
  }

  await db.users.update(user.id, { passwordHash: await hashPassword(parsed.data.newPassword) });
...
router.get('/me/sessions', attachUser, requireAuth, async (req: AuthedRequest, res) => {
  const db = await getDatabase(config.DATABASE_URL);
...
router.delete('/me/sessions/:id', attachUser, requireAuth, async (req: AuthedRequest, res) => {
  const db = await getDatabase(config.DATABASE_URL);
  const sessions = await db.sessions.listActive(req.user!.id);
