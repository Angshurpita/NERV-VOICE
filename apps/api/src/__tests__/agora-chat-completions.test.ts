...
describe('Agora Custom LLM OpenAI-Compatible Endpoint', () => {
  ...
  it('provides health status at /api/agora/status', async () => {
    ...
    expect(body.capabilities).toHaveProperty('voiceRtc');
    expect(body.capabilities).toHaveProperty('cloudAgentConfigured');
  ...
  it('processes speech turn through /api/agora/openai/v1/chat/completions with query callId', async () => {
    ...
    const res = await fetch(`${baseUrl}/api/agora/openai/v1/chat/completions?callId=${encodeURIComponent(call.callId)}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        model: 'echosphere-authoritative-brain',
        messages: [
          { role: 'user', content: 'Where is my order 4852?' },
        ],
      }),
    });
  ...
  it('resolves call by channel name when callId is not provided', async () => {
    ...
    const res = await fetch(`${baseUrl}/api/agora/chat/completions?channel=${encodeURIComponent(channel)}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        messages: [
          { role: 'user', content: 'Can I cancel order 4852?' },
        ],
      }),
    });
  ...
  it('supports Server-Sent Events (SSE) streaming when stream: true is requested', async () => {
    ...
    const res = await fetch(`${baseUrl}/api/agora/openai/v1/chat/completions?callId=${encodeURIComponent(call.callId)}`, {
      ...
      body: JSON.stringify({
        stream: true,
        messages: [
          { role: 'user', content: 'Hello voice agent' },
        ],
      }),
    });
