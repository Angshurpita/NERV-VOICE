...
export const api = {
  ...
  health: () => request<{ status: string; persistence: string; model: string | null }>('/health'),

  scenarios: () =>
    request<{ scenarios: Scenario[]; stats: { orders: number; customers: number } }>(
      '/api/catalogue/scenarios',
    ),

  startCall: (language: LanguageCode, channelName?: string) =>
    request<{ callId: string; caseRef: string; greeting: string; language: LanguageCode; channelName: string }>(
      '/api/calls',
      { method: 'POST', body: JSON.stringify({ language, channelName }) },
    ),
  ...
  publishSignalling: (callId: string, event: string, payload: Record<string, unknown> = {}) =>
    request<{ ok: boolean }>('/api/agora/signalling/publish', {
      method: 'POST',
      body: JSON.stringify({ callId, event, payload }),
    }),

  startCloudAgent: (channelName: string, language: LanguageCode, greeting?: string, callId?: string) =>
    request<{ ok: boolean; agentId?: string; agentRtcUid?: number; message?: string; error?: string }>(
      '/api/agora/agent/start',
      { method: 'POST', body: JSON.stringify({ channelName, language, greeting, callId }) },
    ),

  sayCloudAgent: (channelName: string, text: string, callId?: string) =>
    request<{ ok: boolean; error?: string }>('/api/agora/agent/say', {
      method: 'POST',
      body: JSON.stringify({ channelName, text, callId }),
    }),
  ...
  subscribeSignalling: (callId: string, onEvent: (event: any) => void): (() => void) => {
    try {
      const es = new EventSource(`${BASE}/api/agora/signalling/stream?callId=${encodeURIComponent(callId)}`);
      es.onmessage = (e) => {
        try {
