...
const PRODUCTS = {
  ...
  mattress: p(
    "SKU-FUR-002",
    "Wakefit Orthopaedic Mattress Queen",
    "Furniture",
    12999,
    "REPLACEMENT_ONLY",
  ),
  ...
  cooker: p(
    "SKU-KIT-002",
    "Hawkins Contura 3L Pressure Cooker",
    "Kitchen",
    1850,
    "RETURNABLE",
  ),
...
function build(spec: OrderSpec): Order {
  ...
  const expectedDeliveryAt =
    spec.expectedInDays >= 0
      ? daysAgo(spec.expectedInDays)
      : daysAhead(-spec.expectedInDays);
  const deliveredAt =
    spec.deliveredDaysAgo !== undefined ? daysAgo(spec.deliveredDaysAgo) : null;
  ...
  return {
    ...
    cancelledAt:
      spec.cancelledDaysAgo !== undefined
        ? daysAgo(spec.cancelledDaysAgo)
        : null,
    refundedAt:
      spec.refundedDaysAgo !== undefined ? daysAgo(spec.refundedDaysAgo) : null,
    courier: needsCourier
      ? (spec.courier ?? COURIERS[hash(spec.id) % COURIERS.length]!)
      : null,
    trackingId: needsCourier
      ? `${spec.id}${String(hash(spec.id)).padStart(6, "0").slice(0, 6)}`
      : null,
...
export const SCENARIOS: readonly Scenario[] = [
  {
    ...
    expect:
      "Reads the order back with the name, confirms, explains the delay. No human.",
    escalates: false,
    language: "en",
    tags: ["req6", "req7"],
  },
  {
    ...
    expect:
      "Refuses to guess, reads both numbers back, asks which. Never discloses either order.",
    escalates: false,
    language: "en",
    tags: ["req7", "privacy"],
  },
  {
    ...
    expect:
      "Withholds all order details, asks for the name as entered. No disclosure.",
    escalates: false,
    language: "en",
    tags: ["req7", "privacy"],
  },
  {
    ...
    expect:
      "Verifies, asks the reason, cancels it directly. Explicitly no escalation.",
    escalates: false,
    language: "en",
    tags: ["req6"],
  },
  {
    ...
    expect:
      "Verifies, then hands to a human because it is with the courier today.",
    escalates: true,
    language: "en",
    tags: ["req6.3"],
  },
  {
    ...
    expect:
      "Verifies order + name + window, builds the report, then hands over.",
    escalates: true,
    language: "en",
    tags: ["req6.2"],
  },
  {
    ...
    expect:
      "Escalates with a finding that the 10-day window closed 35 days ago.",
    escalates: true,
    language: "en",
    tags: ["req6.2"],
  },
  ...
  },
  {
    ...
    expect:
      "Two substantive retention attempts, then hands over on the third ask.",
    escalates: true,
    language: "en",
    tags: ["req6.1"],
  },
  {
    ...
    expect:
      "Replies in Devanagari, reads the order number digit by digit, ₹ in Hindi words.",
    escalates: false,
    language: "hi",
    tags: ["req4", "req8"],
  },
  {
    ...
    expect:
      "Recognises the Devanagari request, tries to help twice before handing over.",
    escalates: false,
    language: "hi",
    tags: ["req6.1"],
  },
  {
    ...
    expect:
      "AI resolves it: explains both failed attempts, arranges redelivery.",
    escalates: false,
    language: "en",
    tags: ["req6"],
  },
  {
    ...
    expect:
      "Explains it is already cancelled. No escalation, no duplicate action.",
    escalates: false,
    language: "en",
    tags: ["req6"],
  },
  {
    ...
    expect:
      "AI cancels it — value alone is not a reason to escalate — but flags it.",
    escalates: false,
    language: "en",
    tags: ["req6"],
  },
  ...
  },
  {
    ...
    expect:
      "Says it matches nothing, reads it back, asks them to check a digit.",
    escalates: false,
    language: "en",
    tags: ["req7"],
  },
  {
    ...
    expect:
      "Escalates with per-item findings: keyboard returnable, socks are not.",
    escalates: true,
    language: "en",
    tags: ["req6.2"],
  },
  ...
  },
  {
    ...
    expect:
      "Verifies order and name, confirms out for delivery, escalates to human coordinator.",
    escalates: true,
    language: "en",
    tags: ["req6.3"],
  },
  {
    ...
    expect:
      "Verifies order and customer, notes order is in placed status, cancels immediately with refund confirmation.",
    escalates: false,
    language: "en",
    tags: ["req6"],
  },
  {
    ...
    expect:
      "Verifies order, identity, delivers full report and transfers to human agent.",
    escalates: true,
    language: "en",
    tags: ["req6.2"],
  },
  {
    ...
    expect:
      "Replies in clear native Hindi, reads order ID digit by digit, explains tracking status.",
    escalates: false,
    language: "hi",
    tags: ["req4", "req8"],
  },
  {
    ...
    expect:
      "Verifies order, confirms item is out for delivery today, politely transfers to human coordinator in Hindi.",
    escalates: true,
    language: "hi",
    tags: ["req4", "req6.3"],
  },
  {
    ...
    expect:
      "Verifies order, checks 2 failed delivery attempts, confirms next delivery slot with caller. No escalation.",
    escalates: false,
    language: "en",
    tags: ["req6"],
  },
  {
    ...
    expect:
      "Verifies order, flags grocery category as non-returnable per policy, escalates with finding for supervisor review.",
    escalates: true,
    language: "en",
    tags: ["req6.2"],
  },
...
export function findOrder(orderId: string): Order | null {
  return (
    ORDER_INDEX.get(orderId.trim().toUpperCase()) ??
    ORDER_INDEX.get(orderId.trim()) ??
    null
  );
}
