...
const HI_DIGITS = ['शून्य', 'एक', 'दो', 'तीन', 'चार', 'पाँच', 'छह', 'सात', 'आठ', 'नौ'] as const;
...
const EN_ONES = ['zero','one','two','three','four','five','six','seven','eight','nine'] as const;
...
export function normaliseForSpeech(text: string, lang: LanguageCode): string {
  ...
  out = out.replace(/\b(comma|full stop|period|exclamation mark|question mark|colon|semicolon)\b/gi, '');
  ...
  out = out.replace(/\b([A-Z]{2,4})-([\dA-Z]{2,5})-([\dA-Z]{3,6})\b/gi, (_m, a: string, b: string, c: string) =>
  ...
  for (const sentence of sentences) {
    if (sentence.length > maxChars) {
      ...
      while (rest.length > maxChars) {
        ...
        const cut = Math.max(slice.lastIndexOf(', '), slice.lastIndexOf(' और '), slice.lastIndexOf(' '));
