 ...
 */
const DOMAIN_PATTERNS: ReadonlyArray<{ domain: RestrictedDomain; patterns: RegExp[] }> = [
  {
    domain: 'SELF_HARM',
    patterns: [/\b(suicide|kill myself|end my life|self[- ]harm|want to die)\b/i, /(आत्महत्या|खुदकुशी)/],
  },
  {
...
const GUIDANCE: Record<RestrictedDomain, { guidance: string; escalate: boolean }> = {
  SELF_HARM: {
    guidance:
...
export function classifyRequest(utterance: string): SafetyVerdict {
  for (const { domain, patterns } of DOMAIN_PATTERNS) {
    if (patterns.some((pattern) => pattern.test(utterance))) {
      ...
      return { allowed: false, domain, responseGuidance: guidance, requiresEscalation: escalate };
    }
  }
  return ALLOWED;
}
...
export function screenResponse(
   ...
   */
  const actionClaims: Array<{ pattern: RegExp; action: string; statusWords: string[] }> = [
    {
      pattern: /\b(i have cancelled|i'?ve cancelled|i have canceled|cancellation is (done|complete)|cancel kar diya|रद्द कर दिया)\b/i,
      action: 'cancel_order',
      statusWords: ['cancelled', 'canceled'],
    },
    {
      pattern: /\b(refund (has been|is) (processed|issued|done)|refund kar diya|रिफंड कर दिया)\b/i,
      action: 'issue_refund',
      statusWords: ['refunded'],
    },
    {
      pattern: /\b(i have created (a|your) (case|ticket)|ticket ban gaya|टिकट बना दिया)\b/i,
      action: 'create_support_case',
      statusWords: [],
    },
    {
      pattern: /\b(return (has been|is) (arranged|scheduled|booked)|pickup (has been|is) scheduled)\b/i,
  ...
  for (const { pattern, action, statusWords } of actionClaims) {
    ...
    if (!state.executedActions.includes(action)) {
      violations.push(`Claims "${action}" was performed, but the backend did not execute it.`);
    }
  }
  ...
  const known = Object.values(state.knownOrderStatuses).map((s) => humanStatus(s));
  const claimedStatus = statusWords.find((word) => text.includes(word) && !accountedFor.has(word));
  if (claimedStatus) {
    const backed = known.some((status) => status.includes(claimedStatus));
    if (!backed) {
      violations.push(`States order status "${claimedStatus}" that no lookup supports.`);
    }
  }
  ...
  if (orderId && !orderId.confirmed && orderId.candidates.length > 0) {
    for (const candidate of orderId.candidates) {
      if (
        ...
        violations.push(`Presents unconfirmed order id "${candidate.value}" as confirmed.`);
        break;
      }
    }
  }
