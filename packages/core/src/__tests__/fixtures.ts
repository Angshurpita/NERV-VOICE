...
import { applyLookup, applyNameCheck, applyOrderConfirmation, markReadBack } from '../verification.js';
import type {
  Clock,
...
export function verifiedState(
  ...
  state = observeField(state, 'orderId', order.id, 0.95, `it's ${order.id}`, fixedClock);
  state = applyLookup(state, { outcome: 'found', order, customer }, fixedClock);
  state = observeField(state, 'customerIdentity', customer.name, 0.93, customer.name, fixedClock);
...
export function freshState(intent: IntentKey = 'unknown'): ConversationState {
  ...
  return intent === 'unknown' ? state : { ...state, intent: { value: intent, confidence: 0.9 } };
}
